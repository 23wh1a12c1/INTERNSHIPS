...
if video_ready:
    ...
    st.subheader("📱 Creating smart vertical reel...")
    ...
    try:
        ...
        st.video(vertical_output)
        st.success("✅ Vertical reel created!")

        with open(vertical_output, "rb") as f:
            st.download_button("⬇️ Download Reel", data=f, file_name="smart_reel.mp4", mime="video/mp4")

    except Exception as e:
        st.error(f"❌ Reel creation error: {e}")
